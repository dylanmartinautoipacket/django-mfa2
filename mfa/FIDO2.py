...
def complete_reg(request):
    ...
    try:
        ...
        server = getServer()
        auth_data = server.register_complete(request.session.pop('fido_state'), client_data, att_obj)
        ...
        if (
            ...
            request.session['mfa_reg'] = {
                'method': 'FIDO2',
                'name': getattr(settings, 'MFA_RENAME_METHODS', {}).get('FIDO2', 'FIDO2'),
            }
            return HttpResponse(json.dumps({'status': 'RECOVERY'}))
        else:
    ...
    try:
        ...
        try:
            cred = server.authenticate_complete(
                request.session.pop('fido_state'), credentials, credential_id, client_data, auth_data, signature
            )
        except ValueError:
            return HttpResponse(
                json.dumps(
            ...
            try:
                ...
                pass
            return HttpResponse(json.dumps({'status': 'ERR', 'message': str(excep)}), content_type='application/json')
            ...
            for k in keys:
                if AttestedCredentialData(websafe_decode(k.properties['device'])).credential_id == cred.credential_id:
                    ...
                    if not authenticated:
                        ...
                        return HttpResponse(
                            json.dumps({'status': 'OK', 'redirect': res['location']}),
                            content_type='application/json',
                        )
                    return HttpResponse(json.dumps({'status': 'OK'}), content_type='application/json')
    except Exception as exp:
        return HttpResponse(json.dumps({'status': 'ERR', 'message': exp.message}), content_type='application/json')
